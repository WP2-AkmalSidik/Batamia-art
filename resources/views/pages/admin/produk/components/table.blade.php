            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                @forelse ($produks as $produk)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-400/80 transition-colors duration-200">
                        ...
                        <td class="py-3 px-4 font-semibold text-gray-900 dark:text-gray-300">
                            {{ format_rp($produk->harga) }}</td>
                        <td class="py-3 px-4 font-semibold text-gray-900 dark:text-gray-300">
                            {{ format_berat($produk->berat) }}</td>
                        <td class="py-3 px-4 font-semibold text-gray-900 dark:text-gray-300">{!! $produk->status == true
                            ? '<span id="status-detail" class="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm">Aktif</span>'
                            : '<span id="status-detail" class="px-3 py-1 bg-red-100 text-red-800 rounded-full text-sm">Nonaktif</span>' !!}
                        </td>
                        <td class="py-3 px-4 text-center">
                            <div class="inline-flex space-x-2">
                                ...
                                <button onclick="openDeleteModal({{ $produk->id }},'{{ $produk->nama }}')"
